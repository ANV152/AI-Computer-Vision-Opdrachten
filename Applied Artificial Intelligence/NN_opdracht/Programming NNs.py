# I made this neural network for an exercise during my career. 
# It's basically a neural network made from scratch
# I let zeta (learning rate) decrease the more the network approaches  
import math
...
class Perceptron:
    ...
    def update_w_b(self):
        ...
        """     
        # adjusted_eta = adjust_learning_rate(zeta, self.delta)
        
        for i in range(len(self.weights)):
            input_value = self.inputs[i].output if isinstance(self.inputs[i], Perceptron) else self.inputs[i]
            self.weights[i] += zeta * self.delta * input_value

        # Update bias
        self.bias += zeta * self.delta
        ...
        for epoch in range(epochs):
            for n, y in input_data:  # Iterate over inputs and labels               
                ...
                for i in range(len(self.hidden_Ls[-1])):
                 ...
                 # it has almost an Big O notation of O(n^3) which is a very expensive algorithm
...
#        Hoe schuin de afgeleide wordt

nn = NN()
nn.add_layer(4)  # Hidden layer with 4 perceptrons
# nn.add_layer(1) # hidden layer met 5 perceptrons
nn.add_layer(5)
nn.add_layer(3)  # Output layer with 3 perceptrons
zeta = 2/len(nn.hidden_Ls[-1])
